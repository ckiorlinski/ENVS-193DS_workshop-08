...
drought_exp <- read_xlsx( #file path
...
drought_exp_clean <- drought_exp |> # starting with the drought_exp object
  ...
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |>  
  # making sure water treatment is a factor and reordering lvls
...
slice_sample(
  drought_exp_clean, # displaying rows from dataframe
  n = 10 # 10 rows displayed
)
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water treatment",
       y = "Total mass (g)",
       title = "avg total mass is higher in well watered treatment \n versus drought stressed") +
  # taking out legend
...
model4 <- lm(
  total_g ~ species_name + water_treatment, #formula
  data = drought_exp_clean #data
) 
...
#reference level for species_name: lotus scoperius
#the first level you set in factor levles becomes your reference level!!!
```
...
**What is the best model?**
 ...
 THe model that described the total plant mass (g) best included water treatment and species (F(7, 62) = 27.08, p < 0.001, $\alpha$ = 0.05), but not SLA.
...
model4_predictions <- ggpredict(
  model4, #naming model
  terms = c("species_name", "water_treatment")) |> #naming model predictors
